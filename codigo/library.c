...
void insert_message(struct sensor_message* new,
    ...
    // pushes all the readings to the right
    for(int i = SENSOR_LOG_SIZE - 1; i > 0; i--)
        node->log[i] = node->log[i - 1];

    // adds on the left side
    node->log[0] = new;
...
bool compare_merge(struct sensor_node* node1, struct sensor_node* node2,
    char c){
...
void pub_clients_add(struct pub_clients* array,
    struct public_cli* new){
